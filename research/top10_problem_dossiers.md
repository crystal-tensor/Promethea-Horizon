...
B9 update: T-B9-004n strengthens that checkpoint by eliminating the injected normalized-gap ratio hypothesis. The checked module derives cancellation from a nonzero scale and checks `27/20 != 0`; the refreshed packet binds the current source hashes. The remaining B9 work is still the parameterized all-n construction, support and spectral-width formalization, certificate provenance/replay, and a nontrivial locality-preserving transformation.
