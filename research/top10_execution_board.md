...
Current state: **stage 1m evidence hardening; B2 now has a posterior-calibrated shot-conditioned leakage boundary, while B10-T1 keeps the optimistic B5 response-sampler cost stress negative and B5 keeps production DMRG open**.
...
| B2 | Low-overhead QEC | Stim baseline plus closed reduced-round artifact boundary; T-B2-004 Stim HERALDED_ERASE / DEPOLARIZE1 stress found 10 d=5/d=7 improved rows; T-B2-005 false-positive stress found 5 positive-false-positive d=5/d=7 improved rows at fp=0.001/tick but 0 improved rows by fp=0.003/tick; T-B2-006 posterior-calibrated shot-conditioned boundary evaluates 4 calibration profiles / 1152 profile rows, with 3 profiles preserving some rows, max 4 surviving d=5/d=7 rows in one profile, strict high-purity survival 0, and no robust all-profile survival. | Replace posterior-only calibration with a circuit-level shot-conditioned decoder or hardware-calibrated leakage/flag dataset that preserves d=5/d=7 target-volume rows under strict posterior, missed-leakage, noise-mismatch, and realistic false-positive gates. | Demote the leakage-erasure route if d=5/d=7 target-volume rows disappear under strict posterior thresholds, calibrated false-positive rates, missed-leakage bounds, or noise mismatch. |
...
| Week | Focus |
|---:|---|
| 1 | Freeze B1 proof-log report outline; B2 reduced-round is closed and T-B2-006 has a posterior-calibrated boundary, so convert it into a circuit-level shot-conditioned decoder or calibrated leakage dataset; pair B4 and B8 into one verification lane. |
