#!/usr/bin/env python3
"""T-B3-029/T-B10-015p: record the N2 F1 covariance 10-shard prefix."""
...
MODEL_STATUS = "n2_prefix_ten_of_nineteen_shards_produced_no_row_credit"
SOURCE_TARGET_ID = "T-B3-029/T-B10-015p"
...
EXPECTED_N2_PREFIX_SHARDS = 10
EXPECTED_N2_TOTAL_SHARDS = 19
EXPECTED_TOTAL_SHARDS = 65
EXPECTED_PREFIX_GROUP_COUNT = 5120
...
EXPECTED_PREFIX_HASHES = [
    ...
    "e1d619f5432ed48cc63b42d2d28249841727b63d4202629d20bd7c074d4f6dfc",
    "67b7a54a15156f575e6348aea8cd97d1f9b345760bcabd37f63a453d2924ad88",
    "232daff4619b611f264a3c943f858c6674aac58059ac19bb6517526f27dd640e",
    "3b7f0c37a66fc782466a539da6784ccbf7fa21c98fe085d909ba0f924b2dc723",
    "2dacada8c4f282dcefd8c4b49cc2d4ff8699072c0a16c45d0e7def2a62d477cf",
]
...
def build_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    expected_starts = [index * 512 for index in range(EXPECTED_N2_PREFIX_SHARDS)]
    expected_ends = [(index + 1) * 512 for index in range(EXPECTED_N2_PREFIX_SHARDS)]
    prefix_group_count = sum(item["group_count"] for item in shards if isinstance(item["group_count"], int))
    prefix_pairs = sum(int(item["nonzero_covariance_pair_count"]) for item in shards)
    prefix_variance = sum(float(item["variance_sum"]) for item in shards)
    ...
    requirements = [
        ...
        requirement(
            ...
            "All expected N2 10-prefix shard files exist",
            {"produced_n2_prefix_shard_count": produced_prefix_count, "expected": EXPECTED_N2_PREFIX_SHARDS},
        ),
        requirement(
            "P3",
            all(
        ...
        requirement(
            ...
            and prefix_group_count == EXPECTED_PREFIX_GROUP_COUNT
            and cover_counts == [EXPECTED_N2_COMPILED_COVER_GROUP_COUNT],
            "N2 prefix shards form one contiguous compiled QWC prefix",
            {
                "starts": starts,
                "ends": ends,
                "prefix_group_count": prefix_group_count,
                "compiled_cover_counts": cover_counts,
        ...
        requirement(
            ...
            "N2 10-prefix shard hashes are stable",
            {"shard_hashes": shard_hashes, "n2_prefix10_shard_batch_hash": prefix_hash},
        ),
        requirement(
            "P6",
            all(
    ...
    summary = {
        ...
        "n2_prefix_group_count": prefix_group_count,
        ...
        "n2_prefix10_shard_batch_hash": prefix_hash,
        "n2_prefix_nonzero_covariance_pair_count": prefix_pairs,
        "n2_prefix_variance_sum": prefix_variance,
    ...
    return {
        ...
        "title": "B3/B10 F1 N2 10-Shard Prefix Gate",
        "version": "0.1",
        ...
        "claim_boundary": {
            "what_is_supported": "The first ten N2 compiled-state full-covariance shard outputs exist and form a contiguous compiled QWC prefix.",
...
def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- N2 10-prefix batch hash: `{summary['n2_prefix10_shard_batch_hash']}`",
        ...
        "",
        (
            "The gate records the first ten N2 shard outputs for the F1 route. "
...
def main() -> None:
    ...
    parser.add_argument("--json-output", type=Path, default=Path("results/B3_B10_F1_n2_prefix10_shard_gate_v0.json"))
    parser.add_argument("--markdown-output", type=Path, default=Path("research/B3_B10_F1_n2_prefix10_shard_gate.md"))
