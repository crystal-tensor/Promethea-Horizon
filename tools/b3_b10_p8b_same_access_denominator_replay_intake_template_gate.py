#!/usr/bin/env python3
"""T-B3-043/T-B10-015ad: P8-B same-access denominator replay intake template gate."""
...
MODEL_STATUS = "p8b_denominator_replay_templates_ready_no_denominator_wins"
VERSION = "0.1"
...
def build_template_row(
    ...
    row_hash: str,
    row_index: int,
    p8a_template: dict[str, Any],
    p8_pressure_summary: dict[str, Any],
    ...
    prefilled_values = {
        ...
        "source_pressure_packet_hash": p8_pressure_summary.get("pressure_packet_hash"),
        "p8a_template_hash": p8a_template.get("template_hash"),
        "selected_ci_fci_denominator_replay_hash": submission.get(
            "selected_ci_fci_denominator_replay_hash"
        ),
        "same_access_decision_hash": submission.get("same_access_decision_hash"),
        "optimizer_loop_cost_ledger_hash": submission.get("optimizer_loop_cost_ledger_hash"),
        "b10_access_boundary_hash": submission.get("b10_access_boundary_hash"),
    }
    ...
    denominator_row_accepted = (
        ...
        and submitted.get("same_access_model_valid") is True
        and submitted.get("row_replay_accepted_link") is True
        and submitted.get("denominator_win_count", 0) > 0
        and submitted.get("denominator_decision") == "same_access_win"
    ...
    template = {
        ...
        "submitted_denominator_row_present": submitted_exists,
        "accepted_same_access_denominator_win": denominator_row_accepted,
    }
...
def build_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    p8a_intake = load_json(args.p8a_intake_gate)
    submission = load_json(args.acceptance_packet_submission)
    pressure_summary = p8_pressure["summary"]
    p8a_summary = p8a_intake["summary"]
    p8a_templates = {row["row_id"]: row for row in p8a_intake["row_templates"]}
    ...
    required_keys = [
        ...
        "row_bundle_hash",
        "source_pressure_packet_hash",
        "p8a_template_hash",
        "selected_ci_fci_denominator_replay_hash",
        "same_access_decision_hash",
        "optimizer_loop_cost_ledger_hash",
        "b10_access_boundary_hash",
        "row_acceptance_link_hash",
        "denominator_comparison_table_hash",
        "selected_ci_fci_replay_transcript_hash",
        "access_model_note_hash",
        "denominator_decision_hash",
        "denominator_win_count",
        "denominator_decision",
        "same_access_model_valid",
        "row_replay_accepted_link",
        "claim_boundary_hash",
    ...
    template_rows = [
        build_template_row(
            ...
            row_hash=row_hash,
            row_index=index,
            p8a_template=p8a_templates[row_id],
            p8_pressure_summary=pressure_summary,
    ...
    submitted_denominator_row_count = sum(row["submitted_denominator_row_present"] for row in template_rows)
    accepted_denominator_win_row_count = sum(row["accepted_same_access_denominator_win"] for row in template_rows)
    ...
    requirements = [
        requirement(
            "B1",
            "P8 pressure gate is current and points to P8-B as a ready packet",
            ...
            and "P8-B" in pressure_summary.get("ready_pressure_packet_ids", [])
        ...
        requirement(
            "B2",
            "P8-A intake is current and still zero-credit",
            p8a_intake.get("method") == "b3_b10_p8a_accepted_row_replay_intake_template_gate_v0"
        ...
        requirement(
            "B3",
            "Four P8-B denominator replay templates are generated",
            row_template_count == 4 and all(row_id in p8a_templates for row_id in candidate_ids),
            {"row_template_count": row_template_count, "candidate_row_ids": candidate_ids},
        ),
        requirement(
            "B4",
            "Every template preserves the submitted F1 denominator and access hashes",
            all(
                row["prefilled_values"].get("selected_ci_fci_denominator_replay_hash")
                == submission.get("selected_ci_fci_denominator_replay_hash")
                and row["prefilled_values"].get("same_access_decision_hash")
                == submission.get("same_access_decision_hash")
                and row["prefilled_values"].get("b10_access_boundary_hash")
                == submission.get("b10_access_boundary_hash")
        ...
        requirement(
            "B5",
            "Templates preserve the blocked-before-P8-A zero-credit state",
        ...
        requirement(
            "B6",
            "Submitted P8-B denominator replay artifacts exist for all four candidate rows",
            submitted_denominator_row_count == 4,
            {"submitted_denominator_row_count": submitted_denominator_row_count, "required_rows": 4},
        ),
        requirement(
            "B7",
            "Production denominator replay keys are populated for all submitted P8-B rows",
        ...
        requirement(
            "B8",
            "At least one submitted P8-B row establishes a same-access denominator win",
            accepted_denominator_win_row_count > 0,
            {"accepted_denominator_win_row_count": accepted_denominator_win_row_count},
        ),
        requirement(
            "B9",
            "No P8-B template promotes B3, B10, advantage, or BQP credit",
    ...
    if failed_ids != EXPECTED_FAILED_IDS:
        validation_errors.append(f"unexpected P8-B intake failures: {failed_ids}")
    if accepted_denominator_win_row_count != 0:
        validation_errors.append("P8-B template gate must not fabricate denominator wins")
    if p8a_summary.get("accepted_full_covariance_row_count") != 0:
        validation_errors.append("P8-B template gate must not promote before P8-A row acceptance")

    summary = {
        "intake_template_id": "B3B10-P8B-same-access-denominator-replay-intake",
        ...
        "submitted_denominator_row_count": submitted_denominator_row_count,
        "accepted_denominator_win_row_count": accepted_denominator_win_row_count,
        "accepted_full_covariance_row_count": p8a_summary.get("accepted_full_covariance_row_count"),
        "denominator_win_count": accepted_denominator_win_row_count,
        "production_missing_key_total": production_missing_key_total,
        ...
        "ready_for_p8c_derivative_optimizer_replay": False,
    ...
    return {
        "benchmark_id": "B3_B10",
        "problem_ids": [49, 11],
        "title": "B3/B10 P8-B Same-Access Denominator Replay Intake Template Gate",
        "version": VERSION,
        ...
        "summary": summary,
        "denominator_templates": template_rows,
        "requirements": requirements,
        "claim_boundary": {
            "what_is_supported": (
                "P8-B now has four same-access denominator replay intake templates bound to "
                "the submitted F1 packet and P8-A row templates."
            ),
            "what_is_not_supported": (
                "No P8-B denominator replay artifact has been submitted or accepted. This gate "
                "does not establish denominator wins, accepted rows, B3 reopen, B10-T1 credit, "
                "quantum advantage, or BQP separation."
            ),
            "next_gate": (
                "Submit P8-B denominator replay artifacts with same-access denominator comparison, "
                "selected-CI/FCI replay transcript, access-model note, denominator decision, "
                "row-acceptance link, and claim-boundary hashes."
            ),
            "accepted_full_covariance_row_count": p8a_summary.get("accepted_full_covariance_row_count"),
            "denominator_win_count": accepted_denominator_win_row_count,
            "b3_reopen_ready": False,
...
def write_markdown(payload: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Submitted denominator rows / denominator wins: `{s['submitted_denominator_row_count']}` / `{s['denominator_win_count']}`",
    ...
    for row in payload["denominator_templates"]:
        lines.extend(
            [
                ...
                f"- Submitted denominator row present: `{row['submitted_denominator_row_present']}`",
                f"- Accepted same-access denominator win: `{row['accepted_same_access_denominator_win']}`",
...
def main() -> None:
    ...
    parser.add_argument(
        "--submission-dir",
        type=Path,
        default=Path("results/B3_B10_P8B_same_access_denominator_replay_submissions"),
    )
    parser.add_argument(
        "--json-output",
        type=Path,
    ...
    parser.add_argument(
        "--markdown-output",
        type=Path,
        default=Path("research/B3_B10_P8B_same_access_denominator_replay_intake_template_gate.md"),
    )
