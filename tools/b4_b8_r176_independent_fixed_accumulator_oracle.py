#!/usr/bin/env python3
"""Independently audit R176 without importing Qiskit or its comparator."""
...
METHOD = "b4_b8_r176_independent_fixed_accumulator_oracle_v0"
...
SOURCE_PATH = "results/B4_B8_R176_fixed_superaccumulator_v0.json"
...
RESULT_PATH = "results/B4_B8_R176_independent_fixed_accumulator_oracle_v0.json"
REPORT_PATH = "research/B4_B8_R176_independent_fixed_accumulator_oracle.md"
...
def validate_hash_field(payload: dict[str, Any], field: str, label: str) -> str:
    ...
    if not observed or observed != canonical_hash(body):
        raise ValueError(f"R176 independent {label} hash mismatch")
    return str(observed)
...
def exact_leaf_oracle(
    ...
    if not scored:
        raise ValueError("R176 independent oracle found no feasible mapping")
...
def build_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Independent Check",
            "",
            f"A standard-library-only audit validates `{summary['worker_hashes_valid']}/39` worker hashes and `{summary['row_hashes_valid']}/2400` row hashes. It reproduces `{summary['standard_outcomes_reproduced']}/1728` standard outcomes and independently enumerates all `{summary['small_gap_oracle_count']}/28` sub-ULP exact-oracle cells across source, BigUint, and fixed policies.",
            "",
...
def main() -> int:
    ...
    if (root / RESULT_PATH).exists() or (root / REPORT_PATH).exists():
        raise ValueError("R176 independent oracle evidence already exists")
    ...
    for section in ("source_bindings", "tool_bindings"):
        for binding in contract[section].values():
            ...
            if not path.exists() or file_sha256(path) != binding["sha256"]:
                raise ValueError(
                    f"R176 independent binding mismatch: {binding['path']}"
                )
    manifests = []
    ...
    for manifest in [row for row in manifests if row["kind"] == "small-gap"]:
        for case in manifest["case_summaries"]:
            ...
            if cell in oracle_by_cell and oracle_by_cell[cell] != oracle:
                raise ValueError(f"R176 independent descriptor disagreement: {cell}")
    ...
    requirements = [
        ("P1", len(manifests) == worker_hashes_valid == artifacts_match == 39),
        ("P2", len(rows) == row_hashes_valid == 2400 and case_hashes_valid == 84),
        ("P3", canonical_hash(rows) == source["row_set_hash"]),
        ("P4", standard_outcomes_reproduced == 1728),
        ("P5", len(oracle_by_cell) == 28 and oracle_payload_matches == 84),
        ("P6", all(row["minimizer_count"] == 1 for row in oracle_by_cell.values())),
        ("P7", small_gap_outcomes_reproduced == 672),
        ("P8", performance_cells == source["performance_cells"]),
        ("P9", worker_pairs == source["memory_pairs"] and summary_matches),
        (
            "P10",
            source["requirements_failed"] == 0 and source["requirements_passed"] == 16,
        ),
        (
            "P11",
            "qiskit" not in sys.modules
            and "b4_b8_r176_fixed_superaccumulator_replay" not in sys.modules,
        ),
        ("P12", preregistration_matches),
    ]
    passed = all(value for _, value in requirements)
    result = {
        "title": "B4/B8/B10 R176 independent Fixed-superaccumulator oracle",
        "version": 0,
        "method": METHOD,
        "status": "independent_fixed_superaccumulator_oracle_complete"
        if passed
        else "independent_fixed_superaccumulator_oracle_failed",
        "classification": "standard_library_reproduction_of_integrated_rust_matrix"
        if passed
        else "incomplete",
        "source_target_id": "T-B4-002db/T-B8-003df/T-B10-009cr-r176-oracle",
        ...
        "summary": {
            ...
            "r176_executor_imported": "b4_b8_r176_fixed_superaccumulator_replay"
            in sys.modules,
        ...
        "claim_boundary": {
            "what_is_supported": "a Qiskit-free standard-library reproduction of every frozen R176 mapping expectation, sub-ULP exact oracle, hash, timing ratio, and peak-RSS ratio",
