#!/usr/bin/env python3
"""Audit comparison-policy shadows over the frozen R162 score trace."""
...
METHOD = "b4_b8_r163_comparison_policy_v0"
...
OUT_DIR = "results/B4_B8_R163_comparison_policy"
...
def choose_leaves(index: dict[str, list[list[int]]], expression: str, source_bits: int) -> list[int] | None:
    candidates = index.get(expression, [])
    matching = [bits for bits in candidates if source_fold(bits) == source_bits]
    if len(matching) == 1:
        return matching[0]
    if len(matching) > 1:
...
def validate_payload(payload: dict[str, Any], label: str) -> str:
    ...
    if not payload_hash or payload_hash != canonical_hash(body):
        raise ValueError(f"R163 {label} payload hash mismatch")
    return payload_hash
...
def validate_bindings(root: Path, protocol_payload: dict[str, Any], contract: dict[str, Any]) -> None:
    ...
    if protocol_payload.get("method") != "b4_b8_r163_comparison_policy_protocol_v0":
        raise ValueError("R163 protocol identity mismatch")
    if contract.get("contract_id") != "B4-B8-R163-comparison-policy-contract-v0":
        raise ValueError("R163 contract identity mismatch")
    if contract.get("execution_started") is not False:
        raise ValueError("R163 contract is not unopened")
    if contract["source_bindings"]["protocol"]["payload_hash"] != protocol_hash:
        raise ValueError("R163 protocol binding mismatch")
    for binding_id, binding in contract["source_bindings"].items():
        path = root / binding["path"]
        if not path.exists() or file_sha256(path) != binding["sha256"]:
            raise ValueError(f"R163 source binding mismatch: {binding_id}")
        if "payload_hash" in binding:
            payload = json.loads(path.read_text(encoding="utf-8"))
            if payload.get("payload_hash") != binding["payload_hash"]:
                raise ValueError(f"R163 source payload mismatch: {binding_id}")
    protocol = protocol_payload["protocol"]
    if protocol["r162_result_payload_hash"] != contract["source_bindings"]["r162_result"]["payload_hash"]:
        raise ValueError("R163 R162 result payload binding mismatch")
    if contract_hash == "":
        raise ValueError("unreachable contract hash")


def event_index(events: list[dict[str, Any]]) -> dict[str, list[list[int]]]:
    ...
    for event in events:
        ...
        index.setdefault(expression, []).append(bits)
    return index
...
def build_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "## Result",
        "",
        f"The audit retained `{summary['reconstructable_compare_event_count']}` of `{summary['source_compare_event_count']}` compare events. Disagreements against the source are `{summary['source_vs_policy_disagreement_count']}` for source, compensated, exact-binary64, and tie-aware policies respectively. The result is a comparison-level diagnostic only; it does not prove that any mapping would change under a production rerun.",
        "",
        "## Profile Summary",
        "",
...
def aggregate(root: Path, protocol_payload: dict[str, Any], contract: dict[str, Any], preregistration: dict[str, str]) -> dict[str, Any]:
    ...
    result = {
        "title": "B4/B8 R163 VF2 comparison-policy shadow audit",
        "version": 0,
        "method": METHOD,
        ...
        "classification": "comparison_policy_differences_observed" if any(disagreement[policy] for policy in POLICIES if policy != "source_f64") else "comparison_policies_agree_on_reconstructable_events",
        "source_target_id": "T-B4-002cg/T-B8-003ck/T-B10-009by",
        "upstream_target_id": "T-B4-002cf/T-B8-003cj/T-B10-009bx",
...
def main() -> int:
    ...
    if (root / OUT_DIR).exists() or (root / RESULT_PATH).exists():
        raise ValueError("R163 execution evidence already exists; refusing to overwrite")
