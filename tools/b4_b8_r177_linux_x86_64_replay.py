...
METHOD = "b4_b8_r177_linux_x86_64_replay_v0"
...
RESULT_PATH = "results/B4_B8_R177_linux_x86_64_replay_v0.json"
REPORT_PATH = "research/B4_B8_R177_linux_x86_64_replay.md"
OUT_DIR = "results/B4_B8_R177_linux_x86_64_replay"
ORACLE_PATH = "results/B4_B8_R177_independent_linux_x86_64_oracle_v0.json"
...
def validate_contract(
    ...
    *,
    ...
    if contract.get("execution_started") is not False:
        raise ValueError("R177 contract is not an unopened record")
    ...
    if pre_replay:
        for relative in (OUT_DIR, RESULT_PATH, REPORT_PATH, ORACLE_PATH):
            if (root / relative).exists():
                raise ValueError(f"R177 replay evidence already exists: {relative}")
...
def launch_worker(
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"R177 worker failed: {' '.join(args)}\n"
            f"{completed.stdout}\n{completed.stderr}"
        )
...
def build_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# B4/B8/B10 R177 Linux x86-64 Replay",
            "",
...
def aggregate(
    ...
    build_binding_passed = (
        build.get("status") == "linux_x86_64_accelerator_built_after_preregistration"
    ...
    result.update(
        {
            "title": "B4/B8/B10 R177 Linux x86-64 replay",
            "method": METHOD,
            ...
            "source_target_id": "T-B4-002dd/T-B8-003dh/T-B10-009ct-r177-result",
...
def main() -> int:
    ...
    if platform.system() != "Linux" or platform.machine() not in {"x86_64", "amd64"}:
        raise ValueError("R177 replay requires Linux x86-64")
    if not (root / BUILD_MANIFEST_PATH).exists() or not (root / BINARY_PATH).exists():
        raise ValueError(
            "R177 replay requires the Linux build manifest and accelerator"
        )
    if args.worker_kind:
